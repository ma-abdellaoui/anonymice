      ...
      {events.map((event) => (
        <tr
          ...
          className={`cursor-pointer transition-colors ${
            selectedId === event.id ? "bg-muted" : "hover:bg-muted"
          }`}
        >
          <td className="whitespace-nowrap px-3 py-2 font-mono text-xs text-muted-foreground">{clockTime(event.at)}</td>
          ...
          <td className="px-3 py-2">
            <div className="flex flex-wrap gap-1">
              {totalEntities(event) === 0 && <span className="text-xs text-muted-foreground/50">none</span>}
              {Object.entries(event.entity_counts).map(([entity, count]) => (
                <span key={entity} className="rounded bg-muted px-1.5 py-0.5 font-mono text-[10px] text-muted-foreground">
                  {entity}
                  {count > 1 ? ` ×${count}` : ""}
                </span>
              ))}
          ...
          <td className="px-3 py-2">
            <div className="flex items-center gap-2">
              ...
              <span className="truncate text-xs text-muted-foreground">{outcomeSummary(event)}</span>
            </div>
          </td>
