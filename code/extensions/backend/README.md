...
| env | default | what |
|---|---|---|
| `DETECT_TOKEN` | none | the bearer credential. Required, at least 16 characters |
| `DETECT_TOKEN_PREVIOUS` | none | also accepted, so a rotation has no closed window |
...
| `MAX_BODY_BYTES` | `1048576` | over it is a `413`, meaning "re-split" |
...
produce exactly the silent divergence those rules exist to prevent.
...
normalisation with case preserved, and the client uses it as given. If §5.1 grows a
